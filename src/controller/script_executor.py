"""Safely execute LLM-generated Python scripts on spreadsheet data."""
...
class ScriptExecutor:
    ...
    def __init__(self, script_dir: Optional[str] = None):
        """Initialize the scr            if cell_value not in col_patterns[col_idx]:
                col_patterns[col_idx][row_idx] = cell_value
        
        additional_modifications = []xecutor."""
    ...
    def refresh_spreadsheet_structure(self, session_id: str) -> pd.DataFrame:
        """Placeholder: controller should supply the latest DataFrame; returns None."""
    ...
    def execute_script(self, script: str, spreadsheet_df: pd.DataFrame, file_manager: Optional[FileManager] = None, session_id: Optional[str] = None) -> Tuple[pd.DataFrame, List[List[int]]]:
        """Execute a Python script on spreadsheet data with enhanced session logging."""
        print(f"\n{'='*60}")
        ...
        if session_id:
            try:
                ...
                session_manager.log_terminal_command(
                    session_id,
                    f"🔧 Script Execution: {script[:100]}{'...' if len(script) > 100 else ''}",
                    "🚀 Starting script validation and execution pipeline",
                    "info"
                )
            except ImportError:
                pass
            ...
            if session_id:
                try:
                    ...
                    session_manager.log_terminal_command(
                        session_id,
                        "🔧 Script Validation",
                        f"⚠️ Initial validation failed: {error_message}",
                        "warning"
                    )
                except ImportError:
                    pass
            ...
            if not is_valid:
                ...
                if session_id:
                    try:
                        ...
                        session_manager.log_terminal_command(
                            session_id,
                            "🔧 Advanced Script Fixing",
                            "🤖 Attempting advanced error correction with ScriptFixer",
                            "info"
                        )
                    except ImportError:
                        pass
                ...
                # Save script temporarily to provide a path for the fixer
                try:
                    import uuid
                    sid = str(uuid.uuid4())[:8]
                    os.makedirs(self.script_dir, exist_ok=True)
                    # Temp path uses legacy-like name but in scripts dir
                    pre_path = os.path.join(self.script_dir, f"temp_{sid}.py")
                    with open(pre_path, 'w', encoding='utf-8') as f:
                        f.write(script)
                    script_relative_path = f"src/scripts/temp_{sid}.py"
                except Exception:
                    ...
                    if session_id:
                        try:
                            ...
                            session_manager.log_terminal_command(
                                ...
                                f"🎉 ScriptFixer successfully fixed and executed the script\n📊 Modified {len(modified_cells)} cells",
                                "success"
                            )
                        except ImportError:
                            pass
                    ...
                    if session_id:
                        try:
                            ...
                            session_manager.log_terminal_command(
                                ...
                                f"💥 ScriptFixer could not fix the script: {error_message}",
                                "error"
                            )
                        except ImportError:
                            pass
        ...
        is_safe = self.security_manager.validate_script(
            script, 
            session_id=session_id, 
            script_name=f"execution_{session_id}"
        )
        if not is_safe:
            error_msg = f"Script validation failed due to security concerns"
        ...
        script_path = os.path.join(script_dir, f"{ts_name}_{script_id}.py")
        with open(script_path, 'w', encoding='utf-8') as f:
            f.write(script)

        print(f"💾 Script saved: {script_id}")
        script_relative_path = f"src/scripts/{ts_name}_{script_id}.py"
        ...
        try:
            ...
            if 'iloc cannot enlarge its target object' in error_msg:
                ...
                if max_row_needed >= current_rows:
                    rows_to_add = max_row_needed - current_rows + 10
                ...
                if max_col_needed >= current_cols:
                    cols_to_add = max_col_needed - current_cols + 5
    ...
    def validate_script_safety(self, script: str) -> bool:
        ...
        is_safe = self.security_manager.validate_script(script, script_name="safety_check")
        return is_safe
    ...
    def execute_simple_script(self, script: str, spreadsheet_df: pd.DataFrame, command: str, file_manager: Optional[FileManager] = None, session_id: Optional[str] = None) -> Tuple[pd.DataFrame, List[List[int]]]:
        ...
        # Save script to src/scripts directory for reference
        script_dir = self.script_dir
        os.makedirs(script_dir, exist_ok=True)
        script_path = os.path.join(script_dir, f"{ts_name}_{script_id}.py")
        with open(script_path, 'w', encoding='utf-8') as f:
