...
class ScriptExecutor:
    ...
    def execute_script(self, script: str, spreadsheet_df: pd.DataFrame, file_manager: Optional[FileManager] = None, session_id: Optional[str] = None) -> Tuple[pd.DataFrame, List[List[int]]]:
        ...
        Args:
            ...
            session_id: Optional session ID for enhanced logging and structure refresh
        ...
        if not is_valid:
            ...
            if not is_valid:
                ...
                try:
                    ...
                    pre_path = os.path.join(self.script_dir, f"script_{sid}.py")
                    with open(pre_path, 'w', encoding='utf-8') as f:
                        f.write(script)
                    script_relative_path = f"src/script/script_{sid}.py"
                except Exception:
        ...
        # Validate script for security (after fixing) with session logging
        is_safe, security_message = self.security_manager.validate_script(script, session_id)
        if not is_safe:
            error_msg = f"Script validation failed due to security concerns: {security_message}"
    ...
    def validate_script_safety(self, script: str) -> bool:
        ...
        """
        is_safe, _ = self.security_manager.validate_script(script)
        return is_safe
